...
从 [Releases](https://github.com/yourusername/govm/releases) 页面下载对应平台的预编译二进制文件。

#### Linux

```bash
# 下载（根据你的架构选择）
wget https://github.com/yourusername/govm/releases/latest/download/govm-x86_64-unknown-linux-gnu.tar.gz
...
```bash
# Intel Mac
wget https://github.com/yourusername/govm/releases/latest/download/govm-x86_64-apple-darwin.tar.gz
tar xzf govm-x86_64-apple-darwin.tar.gz
sudo mv govm /usr/local/bin/

# Apple Silicon Mac
wget https://github.com/yourusername/govm/releases/latest/download/govm-aarch64-apple-darwin.tar.gz
...
```powershell
# 下载
Invoke-WebRequest -Uri "https://github.com/yourusername/govm/releases/latest/download/govm-x86_64-pc-windows-msvc.zip" -OutFile "govm.zip"
...
A: 支持 Windows、Linux 和 macOS。支持的架构：
